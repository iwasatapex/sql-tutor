...
function renderPredictPanel(e){const panel=$('predict-panel');const queryDisplay=$('predict-query-display');const predictionInput=$('prediction');const checkBtn=$('check-prediction');if(!panel||!e){if(panel)panel.style.display='none';return}if(e.question_type==='predict'){$('predict-panel').style.display='';$('predict-query-display').textContent=e.expected_query;$('query').value='';if(predictionInput)predictionInput.value='';if(checkBtn)checkBtn.onclick=checkPrediction}else{if(panel)panel.style.display='none'}}
function renderState(payload){renderTopics(payload.topics,payload.selected_topic);renderDifficulty(payload.selected_difficulty);renderQuestionType(payload.question_type);renderExercise(payload.exercise);renderDebugPanel(payload.exercise);renderPredictPanel(payload.exercise);renderStats();renderFeedback(payload.feedback)}
...
async function action(path,opts={}){const throwOnError=!!opts.throwOnError;if(submitInFlight&&!throwOnError)return;setBusy(true);try{state=await api(path,{method:'POST'});window.__sqlTutorState=state;renderExercise(state.exercise);renderStats();renderFeedback(state.feedback);$('query').value='';await refreshProgress();$('status').textContent='Ready'}catch(e){$('status').textContent=e.message;if(throwOnError)throw e}finally{setBusy(false)}}
...
_SUPPORTED_QUESTION_TYPES = ("write", "debug", "predict")
